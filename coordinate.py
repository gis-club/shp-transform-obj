...
def calculate_coordinate(targe_coordinate, center_coordinate):
    """
    ...
    Args:
        targe_coordinate (numpy.ndarray): Target geographic coordinates (longitude, latitude)
        center_coordinate (numpy.ndarray): Center reference coordinates (longitude, latitude)
    ...
    lon = targe_coordinate[0] - center_coordinate[0]
    lat = targe_coordinate[1] - center_coordinate[1]
    ...
    dist_x = distance.geodesic((targe_coordinate[1], center_coordinate[0]), (center_coordinate[1], center_coordinate[0]), ellipsoid='WGS-84').meters
    # Y distance: longitude difference at center latitude
    dist_y = distance.geodesic((center_coordinate[1], targe_coordinate[0]), (center_coordinate[1], center_coordinate[0]), ellipsoid='WGS-84').meters

    # Apply quadrant-based sign corrections
    # Note: X corresponds to Y, Y corresponds to X due to coordinate system mapping
    if not bool(quadrant_x):
        dist_y = 0 - dist_y
    if not bool(quadrant_y):
        dist_x = 0 - dist_x
    ...
    point = rotate_2d(np.array([dist_y, -dist_x]), -90)
